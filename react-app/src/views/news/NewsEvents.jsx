...
export default function NewsEvents({ items }) {
  const source = items?.length ? items : newsEvents;
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "Events") {
      return source.filter((n) => n.category === "Events" || n.category === "Cultural" || n.category === "Academic");
    }
      ...
      {featured && (
        <section className="bulletin-cover-section">
          <div className="container">
            ...
            <Link to={`/events/${featured.slug}`} className="bulletin-cover">
          ...
          {list.length === 0 && !featured ? (
            ...
            <div className="bulletin-list">
              {list.map((item) => {
                ...
                return (
                  <Link key={item.slug} to={`/events/${item.slug}`} className="bulletin-row">
